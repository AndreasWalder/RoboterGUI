...
namespace RoboterGUI
{
    public partial class ControlSerialForm : Form       // Form can be loaded twice!!! COM1, COM2
    {
        ...
        private string parserStateGC = "";                  // keep parser state response [GC:G0 G54 G17 G21 G1 G94 M5 M9 T0 F0.0 S0]
 ...
 //           Application.ThreadException += new System.Threading.ThreadExceptionEventHandler(Application_ThreadException);
        ...
        {
            if (dataField[0].IndexOf("GC") >= 0)            // handle G-Code parser state [GC:G0 G54 G17 G21 G1 G94 M5 M9 T0 F0.0 S0]
            {
            ...
            {   if ((posPause.X != posWorld.X) || (posPause.Y != posWorld.Y) || (posPause.Z != posWorld.Z))
                {
                    ...
                    requestSend(string.Format("G1 G0 X{0:0.0} Y{1:0.0}", posPause.X, posPause.Y).Replace(',', '.'));  // restore last position
