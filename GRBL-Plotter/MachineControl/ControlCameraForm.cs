...
/*            if (teachTP1)
        ...
        private void pictureBoxVideo_Click(object sender, MouseEventArgs e)//, EventArgs e)
        {
            ...
            else if (teachingBot)
            {
                ...
                OnRaiseXYEvent(new XYEventArgs(0, 1, realPosition, "G1")); // move relative and slow
        ...
        {
            OnRaiseXYEvent(new XYEventArgs(0,0, 0, "G1")); // set new coordinates
        }
        // send event to teach Marker offset
        private void btnSetOffsetMarker_Click(object sender, EventArgs e)
        {
            OnRaiseXYEvent(new XYEventArgs(0, 1, actualPosMarker, "G1"));        // set new coordinates
        }
        // sent event to apply offset
        private void btnCamCoordTool_Click(object sender, EventArgs e)
        {   if (cBCamCoordMove.Checked)
                OnRaiseXYEvent(new XYEventArgs(0, 1, (xyPoint)actualPosWorld, "G54; G0G1"));  // switch coord system and move
            else
        ...
        {   if (cBCamCoordMove.Checked)
                OnRaiseXYEvent(new XYEventArgs(0, 1, (xyPoint)actualPosWorld, "G59; G0G1"));  // switch coord system and move
            else
        ...
        private void btnAutoCenter_Click(object sender, EventArgs e)
        {
            if (shapeFound)
            {
                ...
                OnRaiseXYEvent(new XYEventArgs(0, 1, tmp/2, "G1")); // move relative and slow
