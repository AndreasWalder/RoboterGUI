...
namespace RoboterGUI
{
    public partial class Control2ndGRBL : Form
    {
        ...
        { sendCommand("G1 X0 F" + joystickXYSpeed[5].ToString(), true); }
        private void btnJogY_Click(object sender, EventArgs e)
        { sendCommand("G1 Y0 F" + joystickXYSpeed[5].ToString(), true); }
        private void btnJogZ_Click(object sender, EventArgs e)
        { sendCommand("G1 Z0 F" + joystickZSpeed[5].ToString(), true); }
        ...
        { if (_serial_form2.isGrblVers0) sendCommand("G1G1F100"); }
        private void virtualJoystickXY_Leave(object sender, EventArgs e)
        { if (_serial_form2.isGrblVers0) sendCommand("G1"); }
        private void virtualJoystickX_JoyStickEvent(object sender, JogEventArgs e)
        {
            ...
            if (speed > 0)
            {
                String s = String.Format("G1 X{0} F{1}", strZ, speed).Replace(',', '.');
                sendCommand(s, true);
            }
        }
        private void virtualJoystickY_JoyStickEvent(object sender, JogEventArgs e)
        {
            ...
            if (speed > 0)
            {
                String s = String.Format("G1 Y{0} F{1}", strZ, speed).Replace(',', '.');
                sendCommand(s, true);
            }
        }
