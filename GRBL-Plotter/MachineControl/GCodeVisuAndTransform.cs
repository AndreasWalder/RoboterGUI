...
namespace GRBL_Plotter
{
    ...
    {   public enum translate { None, ScaleX, ScaleY, Offset1, Offset2, Offset3, Offset4, Offset5, Offset6, Offset7, Offset8, Offset9, MirrorX, MirrorY };
        ...
        private class gcodeLine
        {
            ...
            public bool isdistanceModeG1;  // G1,91
            ...
            {   resetAll(); lineNumber = tmp.lineNumber; codeLine = tmp.codeLine; motionMode = tmp.motionMode;
                isdistanceModeG1 = tmp.isdistanceModeG1; ismachineCoordG53 = tmp.ismachineCoordG53;
            ...
            /// <summary>
            /// Reset coordinates and set G1, M5, M9
            /// </summary>
            public void resetAll()
            {   motionMode = 0; isdistanceModeG1 = true; ismachineCoordG53 = false; isSubroutine = false; actualPos.X = 0; actualPos.Y = 0; actualPos.Z = 0; distance = -1;
        ...
        private void calcAbsPosition(gcodeLine newLine, gcodeLine oldLine)
        {
            ...
            {   if (newLine.x != null)
                {   if (newLine.isdistanceModeG1)  // absolute move
        ...
        {   switch (Char.ToUpper(cmd))
            {   case 'X':
                ...
                case 'G':
                    ...
                    if (value == 90)
                    { parseLine.isdistanceModeG1 = true; }

                    if (value == 91)
                    {   parseLine.isdistanceModeG1 = false;        
                        containsG1 = true;  }
                    break;
                case 'M':
                    if (value >= 3 && value <= 5)
        ...
        public string transformGCodeOffset(double x, double y, translate shiftToZero)
        {
            ...
            if (containsG1)    // relative move: insert rapid movement before pen down, to be able applying offset
            {
                newLine.resetAll();
                int i,k;
                bool foundG1 = false;
                for (i = 0; i < gcodeList.Count; i++)       // find first relative move
                {   if ((!gcodeList[i].isdistanceModeG1) && (!gcodeList[i].isSubroutine) && (gcodeList[i].motionMode == 0) && (gcodeList[i].z != null))       
                    { foundG1 = true; break; }
                }
                if (foundG1)
                {
            ...
            foreach (gcodeLine gcline in gcodeList)
            {
                ...
                {
                    if (gcline.isdistanceModeG1)           // absolute move: apply offset to any XY position
                    {
                        if (gcline.x != null)
