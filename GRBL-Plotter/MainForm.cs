...
namespace RoboterGUI
{
    ...
    public partial class MainForm : Form
    {
        ...
        private void MainForm_Load(object sender, EventArgs e)
        {
            ...
            if (args.Length > 1)
            { loadFile(args[1]); }
            label_wa.Text = "500,000";
            label_wb.Text = "0,000";
            label_wc.Text = "100,000"; 
        ...
        private void OnRaisePosEvent(object sender, PosEventArgs e)
        {
            ...
            label_we.Text = string.Format("{0:0.000}", posWorld.A);
            ...
            if (flagResetOffset)
            {
                ...
                if (ctrl4thAxis)
                    sendCommand(String.Format("G1 X{0} Y{1} Z{2} {3}{4}F{5}", x, y, z, ctrl4thName, a, Properties.Settings.Default.importGCXYFeed).Replace(',', '.'));
                else
                    sendCommand(String.Format("G1 X{0} Y{1} Z{2} F{3}", x, y, z, Properties.Settings.Default.importGCXYFeed).Replace(',', '.'));
            ...
            if (machineStatus != lastMachineStatus)
            {
                switch (machineStatus)
                {
                    case grblState.idle:
                        ...
                        if (!isStreaming)                       // update drawing if G1 is used
        ...
        private void processParserState(pState cmd)//string cmd)
        {
            if (cmd.changed)
            {
                ...
                cBGreifer.Checked = (cmd.spindle <= 4) ? true : false;  // M3, M4 start, M5 stop
...
//                        ControlPowerSaving.SuppressStandby();
        ...
        private void MainTimer_Tick(object sender, EventArgs e)
        {
            ...
            {
                if (updateDrawingPath && visuGCode.containsG1Command())
                {
                    //redrawGCodePath();
        ...
        private void OnRaiseCameraClickEvent(object sender, XYEventArgs e)
        {
            ...
            else
            {
                ...
                foreach (string cmd in line)
                {
                    ...
                    else if ((cmd.Trim().IndexOf("G1") == 0) || (cmd.Trim().IndexOf("G1") == 0))      // no G0 G1, then jogging
                    {
 ...
 //*               blockFCTB_Events = true;
        ...
        private int virtualJoystickE_lastIndex = 1;
        private int virtualJoystickA_lastIndex = 1;
        private int virtualJoystickB_lastIndex = 1;
        private int virtualJoystickC_lastIndex = 1;
        ...
        { 
            ...
            double posX = Double.Parse(label_wx.Text);
            double X = Double.Parse(strX);
            double ergX = posX + X;
            strX = gcode.frmtNum(ergX);
            String strY = gcode.frmtNum(joystickXYStep[indexY] * dirY);
            double posY = Double.Parse(label_wy.Text);
            double Y = Double.Parse(strY);
            double ergY = posY + Y;
            strY = gcode.frmtNum(ergY);
            String s = "";
            if (speed > 0)
        ...
        private void virtualJoystickZ_move(int index_Z)
        {
            ...
            double posZ = Double.Parse(label_wz.Text);
            double Z = Double.Parse(strZ);
            double ergZ = posZ + Z;
            if (ergZ < 0) { return; }
            strZ = gcode.frmtNum(ergZ);
        ...
        private void virtualJoystickE_JoyStickEvent(object sender, JogEventArgs e)
        {
            int indexE = Math.Abs(e.JogPosY);
            int dirE = Math.Sign(e.JogPosY);
            int speed = int.Parse(tBSpeed.Text);
            virtualJoystickE_lastIndex = indexE;
            String strE = gcode.frmtNum(joystickZStep[indexE] * dirE);
            double posE = Double.Parse(label_we.Text);
            double E = Double.Parse(strE);
            double ergE = posE + E;
            strE = gcode.frmtNum(ergE);
            if (speed > 0)
            {
                label_we.Text = strE;
                String s = String.Format("G1 E{0} F{1}", strE, speed).Replace(',', '.');
                sendCommand(s, true);
            }
        }
        ...
        {
            int indexA = Math.Abs(e.JogPosY);
            int dirA  = Math.Sign(e.JogPosY) * 5;
            int speed = int.Parse(tBSpeed.Text);
            virtualJoystickA_lastIndex = indexA;
            String strA = gcode.frmtNum(joystickZStep[indexA] * dirA);
            double posA = Double.Parse(label_wa.Text);
            double A = Double.Parse(strA);
            double ergA = posA + A;
            if (ergA < 200) { return; }
            if (ergA > 900) { return; }
            strA = gcode.frmtNum(ergA);
            if (speed > 0)
            {
                label_wa.Text = strA;
                String s = String.Format("G1 A{0} F{1}", strA, speed).Replace(',', '.');
                sendCommand(s, true);
            }
        }
        ...
        {
            int indexB = Math.Abs(e.JogPosY);
            int dirB = Math.Sign(e.JogPosY) * 5;
            int speed = int.Parse(tBSpeed.Text);
            virtualJoystickB_lastIndex = indexB;
            String strB = gcode.frmtNum(joystickZStep[indexB] * dirB);
            double posB = Double.Parse(label_wb.Text);
            double B = Double.Parse(strB);
            double ergB = posB + B;
            if (ergB < 0) { return; }
            if (ergB > 950) { return; }
            strB = gcode.frmtNum(ergB);
            if (speed > 0)
            {
                label_wb.Text = strB;
                String s = String.Format("G1 B{0} F{1}", strB, speed).Replace(',', '.');
                sendCommand(s, true);
            }
        }
        ...
        {
            int indexC = Math.Abs(e.JogPosY);
            int dirC = Math.Sign(e.JogPosY) * 5;
            int speed = int.Parse(tBSpeed.Text);
            virtualJoystickC_lastIndex = indexC;
            String strC = gcode.frmtNum(joystickZStep[indexC] * dirC);
            double posC = Double.Parse(label_wc.Text);
            double C = Double.Parse(strC);
            double ergC = posC + C;
            if (ergC < 100) { return; }
            if (ergC > 700) { return; }
            strC = gcode.frmtNum(ergC);
            if (speed > 0)
            {
                label_wc.Text = strC;
                String s = String.Format("G1 C{0} F{1}", strC, speed).Replace(',', '.');
                sendCommand(s, true);
            }
        }
        ...
        { sendCommand("G0 X0 F1000"); label_wx.Text = "0,000"; }
        private void btnZeroY_Click(object sender, EventArgs e)
        { sendCommand("G0 Y0 F1000"); label_wy.Text = "0,000"; }
        private void btnZeroZ_Click(object sender, EventArgs e)
        { sendCommand("G0 Z0 F1000"); label_wz.Text = "0,000"; }
        private void btnZeroE_Click(object sender, EventArgs e)
        { sendCommand("G0 E0 F1000"); label_we.Text = "0,000"; }
        private void btnZeroA_Click(object sender, EventArgs e)
        { sendCommand("G0 A500 F1000"); label_wa.Text = "500,000"; }
        private void btnZeroB_Click(object sender, EventArgs e)
        { sendCommand("G0 B0 F1000"); label_wb.Text = "0,000"; }
        private void btnZeroC_Click(object sender, EventArgs e)
        { sendCommand("G0 C100 F1000"); label_wc.Text = "100,000"; }
        private void btnZeroXY_Click(object sender, EventArgs e)
        { sendCommand("G0 X0 Y0 F1000"); label_wx.Text = "0,000"; label_wy.Text = "0,000"; }
        private void btnZeroXYZ_Click(object sender, EventArgs e)
        { sendCommand("G0 X0 Y0 Z0 E0 F1000"); label_wx.Text = "0,000"; label_wy.Text = "0,000"; label_wz.Text = "0,000"; label_we.Text = "0,000"; }
        private void btnZeroABC_Click(object sender, EventArgs e)
        {
         sendCommand("G0 A500 F1000"); label_wa.Text = "500,000";
            Thread.Sleep(500);
         sendCommand("G0 B0 F1000"); label_wb.Text = "0,000";
        ...
        private void virtualJoystickE_Load(object sender, EventArgs e)
        {

        }

        private void virtualJoystickA_Load(object sender, EventArgs e)
        {

        }

        private void virtualJoystickZ_Load(object sender, EventArgs e)
        {
        ...
        private void gB_Jogging_Enter(object sender, EventArgs e)
        {

        }

        private void btnTeachen_Click(object sender, EventArgs e)
        {
            if (fCTBCode.Text == "Paste GCode hier")
            {
                fCTBCode.Text = "";
            }
            String Teach = "G0 " + "X" + label_wx.Text + " Y" + label_wy.Text + " Z" + label_wz.Text + " E" + label_we.Text + " A" + label_wa.Text + " B" + label_wb.Text + " C" + label_wc.Text;
            fCTBCode.InsertText(Teach);
        }

        private void btnPause300_Click(object sender, EventArgs e)
        {
            if (fCTBCode.Text == "Paste GCode hier")
            {
                fCTBCode.Text = "";
            ...
            String Pause = "P 300";
            fCTBCode.InsertText(Pause);
        }

        private void btnPause1000_Click(object sender, EventArgs e)
        {
            if (fCTBCode.Text == "Paste GCode hier")
            {
                fCTBCode.Text = "";
        ...
        private void btnPause2000_Click(object sender, EventArgs e)
        {
            if (fCTBCode.Text == "Paste GCode hier")
            {
                fCTBCode.Text = "";
